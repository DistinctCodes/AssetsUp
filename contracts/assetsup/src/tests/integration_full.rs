...
fn test_governance_and_detokenization_workflow() {
    ...
    assert_eq!(client.proposal_passed(&asset_id, &proposal_id), true);
    
    // Execute detokenization
    client.execute_detokenization(&asset_id, &proposal_id);
    
    // Verify asset is detokenized
    assert_eq!(client.is_detokenization_active(&asset_id), false);
}
...
fn test_transfer_restrictions_workflow() {
    ...
    // Verify whitelist
    assert_eq!(client.is_whitelisted(&asset_id, &investor1), true);
    assert_eq!(client.is_whitelisted(&asset_id, &investor2), false);
}
...
fn test_token_locking_workflow() {
    ...
    assert_eq!(client.is_tokens_locked(&asset_id, &investor), true);
    
    // Unlock tokens
    client.unlock_tokens(&asset_id, &investor);
    
    // Verify tokens are unlocked
    assert_eq!(client.is_tokens_locked(&asset_id, &investor), false);
...
fn test_admin_operations_workflow() {
    ...
    assert_eq!(client.is_authorized_registrar(&registrar), true);
    ...
    assert_eq!(client.is_authorized_registrar(&admin), false);
    
    // Verify new admin is authorized registrar
    assert_eq!(client.is_authorized_registrar(&new_admin), true);
}

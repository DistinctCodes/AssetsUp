...
pub fn set_registry(e: &Env, registry: &Address) {
    e.storage().persistent().set(&DataKey::AssetRegistry, registry);
}

pub fn next_request_id(e: &Env) -> u64 {
    e.storage().persistent().get(&DataKey::NextRequestId).unwrap_or(1)
}

pub fn bump_request_id(e: &Env) -> u64 {
    let id = next_request_id(e);
    e.storage().persistent().set(&DataKey::NextRequestId, &(id + 1));
    id
}

pub fn requests_map(e: &Env) -> Map<u64, TransferRequest> {
    e.storage().persistent().get(&DataKey::Requests).unwrap_or(Map::new(e))
}
...
pub fn rules_map(e: &Env) -> Map<BytesN<32>, ApprovalRule> {
    e.storage().persistent().get(&DataKey::Rules).unwrap_or(Map::new(e))
}
...
pub fn pending_approvals_map(e: &Env) -> Map<u64, Vec<Address>> {
    e.storage().persistent().get(&DataKey::PendingApprovals).unwrap_or(Map::new(e))
}
...
pub fn asset_pending_map(e: &Env) -> Map<BytesN<32>, u64> {
    e.storage().persistent().get(&DataKey::AssetPendingRequest).unwrap_or(Map::new(e))
}

pub fn set_asset_pending_map(e: &Env, m: &Map<BytesN<32>, u64>) {
    e.storage().persistent().set(&DataKey::AssetPendingRequest, m);
}

pub fn asset_history_map(e: &Env) -> Map<BytesN<32>, Vec<u64>> {
    e.storage().persistent().get(&DataKey::AssetHistory).unwrap_or(Map::new(e))
}

...
const MOCK_NOTES: Note[] = [
  {
    id: "1",
    author: "Alice M.",
    text: "Sent for repair. @bob.k please follow up with vendor.",
    timestamp: "2024-01-20T10:00:00Z",
  },
  {
    id: "2",
    author: "Bob K.",
    text: "Will check @carol.s for parts availability.",
...
export default function AssetNotesPage() {
  ...
  const handleChange = (val: string) => {
    ...
    if (
      atIdx >= 0 &&
      atIdx === val.length - 1 - val.slice(atIdx + 1).length + atIdx
    ) {
  ...
  const insertMention = (user: string) => {
    const newText = text.slice(0, mentionStart) + `@${user} `;
    setText(newText);
    setSuggestions([]);
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Asset Notes</h1>

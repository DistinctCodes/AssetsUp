...
const MOCK: Location[] = [
  { id: "1", name: "HQ Building", type: "building", parent: null, assetCount: 120 },
...
function LocationRow({ loc, depth }: { loc: Location; depth: number }) {
  ...
  return (
    <>
      <tr className="border-b border-gray-100 hover:bg-gray-50">
        <td className="px-4 py-3">
          <div className="flex items-center" style={{ paddingLeft: ${depth * 20}px }}>
            {hasChildren
              ? <button onClick={() => setOpen(o => !o)} className="mr-1 text-gray-400">{open ? <ChevronDown size={14}/> : <ChevronRight size={14}/>}</button>
      ...
      {open && children.map(c => <LocationRow key={c.id} loc={c} depth={depth + 1} />)}
    </>
  )
}
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-bold text-gray-900">Locations</h1><p className="text-sm text-gray-500 mt-1">Hierarchical location tree with CRUD</p></div>
        <Button><Plus size={16} className="mr-1.5" />Add Location</Button>
      </div>
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <table className="w-full text-sm">
          <thead><tr className="border-b border-gray-200 bg-gray-50">
            {["Name","Type","Assets","Actions"].map(h=><th key={h} className="text-left px-4 py-3 font-medium text-gray-500">{h}</th>)}
          </tr></thead>
          <tbody>{roots.map(r=><LocationRow key={r.id} loc={r} depth={0}/>)}</tbody>
        </table>
      </div>
    </div>
  )
